...
`,_d=Y.createContext(void 0),kd="portfolio-theme-mode",zg=()=>{if(typeof window<"u"){const b=localStorage.getItem(kd);if(b==="dark"||b==="light")return b}return"dark"},wg=({children:b})=>{const[T,U]=Y.useState(zg);Y.useEffect(()=>{document.documentElement.setAttribute("data-theme",T),localStorage.setItem(kd,T)},[T]);const p=()=>{U(j=>j==="dark"?"light":"dark")};return o.jsx(_d.Provider,{value:{mode:T,toggleTheme:p,isDark:T==="dark"},children:b})},Ag=()=>{const b=Y.useContext(_d);if(!b)throw new Error("useTheme must be used within a ThemeProvider");return b},qd=Ct`
  0% {
...
`,nm={initial:{opacity:1},animate:{opacity:1},exit:{opacity:0,transition:{duration:.2,ease:"easeOut"}}},im={initial:{opacity:0,scale:1.5},animate:{opacity:1,scale:1,transition:{duration:1.5}},exit:{opacity:0,scale:.95,transition:{duration:.2}}},cm={initial:{},animate:{},exit:{scale:.9,opacity:0,transition:{duration:.2,ease:"easeOut"}}},um={initial:{opacity:0,scale:.5},animate:{opacity:1,scale:1,transition:{duration:.8,ease:[.25,.1,.25,1]}}},om={initial:{opacity:0,scale:0,rotate:-180},animate:{opacity:1,scale:1,rotate:0,transition:{type:"spring",stiffness:100,damping:15,delay:.2}}},sm={initial:{opacity:0,scale:.3,rotateY:-90,filter:"blur(10px)"},animate:{opacity:1,scale:1,rotateY:0,filter:"blur(0px)",transition:{type:"spring",stiffness:80,damping:12,delay:.4}}},rm=({onComplete:b})=>{const[T,U]=Y.useState("animate"),p=Array.from({length:20},(j,B)=>({id:B,x:Math.random()*100,y:Math.random()*100,size:Math.random()*4+2,color:B%3===0?"rgba(42, 157, 143, 0.4)":B%3===1?"rgba(59, 180, 177, 0.3)":"rgba(67, 97, 165, 0.3)",duration:Math.random()*3+2,delay:Math.random()*2}));return Y.useEffect(()=>{const j=setTimeout(()=>{U("exit")},2200);return()=>clearTimeout(j)},[]),Y.useEffect(()=>{T==="exit"&&b()},[T,b]),o.jsxs($g,{variants:nm,initial:"initial",animate:T,children:[o.jsx(Jg,{variants:im,initial:"initial",animate:T}),o.jsx(Wg,{initial:{opacity:1},animate:{opacity:T==="exit"?0:1},transition:{duration:.2},children:p.map(j=>o.jsx(Fg,{$size:j.size,$color:j.color,style:{left:`${j.x}%`,top:`${j.y}%`},initial:{opacity:0,scale:0},animate:{opacity:[0,.8,0],scale:[0,1,0],y:[0,-100]},transition:{duration:j.duration,delay:j.delay,repeat:1/0,repeatDelay:Math.random()*2}},j.id))}),o.jsx(Ig,{variants:cm,initial:"initial",animate:T,children:o.jsxs(Pg,{children:[o.jsx(tm,{variants:um,initial:"initial",animate:"animate"}),o.jsx(vo,{}),o.jsx(vo,{}),o.jsx(vo,{}),o.jsx(em,{variants:om,initial:"initial",animate:"animate"}),o.jsxs("picture",{children:[o.jsx("source",{srcSet:"/images/logo-optimized.webp",type:"image/webp"}),o.jsx(am,{src:"/images/logo-optimized.png",alt:"PN",width:80,height:80,loading:"eager",decoding:"async",variants:sm,initial:"initial",animate:"animate"})]}),o.jsx(lm,{initial:{opacity:0},animate:{opacity:1},transition:{delay:.8,duration:.5}})]})})]})},fm=d.canvas`
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
...
`,Dd=[{r:42,g:157,b:143},{r:59,g:180,b:177},{r:67,g:97,b:165},{r:123,g:167,b:217}],dm=()=>{const b=Y.useRef(null),T=Y.useRef([]),U=Y.useRef(0),p=Y.useRef({x:window.innerWidth/2,y:window.innerHeight/2});return Y.useEffect(()=>{const j=b.current;if(!j)return;const B=j.getContext("2d");if(!B)return;const E=()=>{const D=window.devicePixelRatio||1;j.width=window.innerWidth*D,j.height=window.innerHeight*D,j.style.width=`${window.innerWidth}px`,j.style.height=`${window.innerHeight}px`,B.scale(D,D)};E(),window.addEventListener("resize",E);const G=4;T.current=[];for(let D=0;D<G;D++)T.current.push({x:window.innerWidth/2,y:window.innerHeight/2,size:60+D*20,color:Dd[D%Dd.length],speed:.1-D*.015,offset:D*.4});let k=0;const V=()=>{k+=.016,B.clearRect(0,0,j.width,j.height);const D=T.current,$=p.current;for(let Yt=D.length-1;Yt>=0;Yt--){const I=D[Yt],Ot=Math.sin(k*1.5+I.offset*4)*15,ft=Math.cos(k*1.5+I.offset*4)*15;I.x+=($.x+Ot-I.x)*I.speed,I.y+=($.y+ft-I.y)*I.speed;const J=I.size+Math.sin(k*1.2+I.offset*2)*8,bt=B.createRadialGradient(I.x,I.y,0,I.x,I.y,J),{r:pt,g:jt,b:Jt}=I.color;bt.addColorStop(0,`rgba(${pt}, ${jt}, ${Jt}, 0.25)`),bt.addColorStop(.4,`rgba(${pt}, ${jt}, ${Jt}, 0.1)`),bt.addColorStop(.7,`rgba(${pt}, ${jt}, ${Jt}, 0.03)`),bt.addColorStop(1,`rgba(${pt}, ${jt}, ${Jt}, 0)`),B.beginPath(),B.arc(I.x,I.y,J,0,Math.PI*2),B.fillStyle=bt,B.fill()}U.current=requestAnimationFrame(V)},xt=D=>{p.current={x:D.clientX,y:D.clientY}};return window.addEventListener("mousemove",xt),U.current=requestAnimationFrame(V),()=>{window.removeEventListener("resize",E),window.removeEventListener("mousemove",xt),cancelAnimationFrame(U.current)}},[]),o.jsx(fm,{ref:b})},pm=Ct`
  0% {
...
`,gb=d(v.p)`
  ...
  color: var(--text-secondary);
  transition: color 0.4s ease;
`,mb=({data:b})=>{const T=Y.useRef(null),U=ye(T,{once:!0,margin:"-100px"});return o.jsx(ub,{id:"educationalqualifications",ref:T,children:o.jsxs(ob,{children:[o.jsx(Sa,{number:"07",title:"Education",inView:U}),o.jsx(sb,{children:b.timeline.map((p,j)=>o.jsxs(rb,{initial:{opacity:0,x:-30,filter:"blur(4px)"},whileInView:{opacity:1,x:0,filter:"blur(0px)"},viewport:{once:!0,margin:"-50px"},transition:{type:"spring",stiffness:100,damping:20,delay:j*.1},whileHover:{x:16,transition:{type:"spring",stiffness:400,damping:25}},children:[o.jsx(fb,{children:p.date}),o.jsxs(db,{children:[o.jsx(pb,{children:p.title}),o.jsx(gb,{children:p.subTitle})]})]},j))})]})})},Xd=Ct`
  0% {
    background-position: 0% 50%;
  }
  50% {
